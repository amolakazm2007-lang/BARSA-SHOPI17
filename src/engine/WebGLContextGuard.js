...
export class WebGLContextGuard extends EventTarget {
  constructor({ canvas, logger = console } = {}) {
    ...
    this._onLost = (event) => {
      ...
      this.lost = true;
      const error = new BarsaError('WEBGL_CONTEXT_LOST', 'WebGL context lost', { recoverable: true });
      this.logger.error?.('[BARSA][webgl-context-lost]', error);
      this.dispatchEvent(new CustomEvent('lost', { detail: error }));
    };
    this._onRestored = () => {
      this.lost = false;
      this.logger.warn?.('[BARSA][webgl-context-restored]');
  ...
  assertAvailable() {
    if (this.lost) throw new BarsaError('WEBGL_CONTEXT_LOST', 'WebGL context is unavailable', { recoverable: true });
  }
