...
export class PeriodicResumeVerifier {
  constructor({ storage, sessionId, timeoutMs = 10000, logger = console } = {}) {
  ...
  async verify(frameCount) {
    ...
    if (count <= 0 || count === this.lastVerifiedFrame) return { skipped: true, frameCount: count };
    ...
    if (!checkpoint) throw new BarsaError('CHECKPOINT_MISSING', `Checkpoint missing at durable frame ${count}`, { details: { sessionId: this.sessionId, frameCount: count } });
    const durableFrames = Number(checkpoint.durableEncodedFrames ?? checkpoint.framesWritten ?? 0) || 0;
    if (durableFrames < count) {
      throw new BarsaError('CHECKPOINT_NOT_DURABLE', `Checkpoint durable frame count ${durableFrames} trails durable writer frame ${count}`, {
        recoverable: true,
    ...
    if (typeof this.storage.verifySessionIntegrity === 'function') {
      ...
      if (integrity === false || integrity?.ok === false) {
        throw new BarsaError('CHECKPOINT_CORRUPT', `Resume integrity verification failed at durable frame ${count}`, {
          details: { sessionId: this.sessionId, frameCount: count, integrity },
        });
      }
    }
    this.lastVerifiedFrame = count;
    this.logger.info?.('[BARSA][resume-integrity-verified]', { sessionId: this.sessionId, frameCount: count, durableFrames });
