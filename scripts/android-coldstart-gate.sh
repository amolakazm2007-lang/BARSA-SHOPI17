...
start_activity_command() {
  ...
  if ! wait_for_adb_device 12 2; then return 1; fi
  set +e
  timeout 12s adb shell am start -n "$ACTIVITY" >> "$log" 2>&1
  status=$?
  set -e
...
wait_for_app_ready() {
  ...
  local resumed_seen=0
  local activities_file="reports/android-activities-${pass}.txt"
  local activity_block="reports/android-activity-block-${pass}.txt"
  local windows_file="reports/android-windows-${pass}.txt"

  for _ in $(seq 1 35); do
    if [[ -n "${EMU_PID:-}" ]] && ! kill -0 "$EMU_PID" 2>/dev/null; then return 4; fi
    if ! wait_for_adb_device 2 1; then sleep 1; continue; fi
    ...
    if [[ -n "$pid" ]]; then
      ...
      if grep -Eq "(topResumedActivity=|ResumedActivity:|mResumedActivity=).*${PACKAGE}/\.MainActivity" "$activities_file"; then
        resumed_seen=1
        awk -v pkg="$PACKAGE" '
          $0 ~ "ActivityRecord\\{.*" pkg "/\\.MainActivity" {capture=1; count=0}
          capture {print; count++}
          capture && count>=100 {exit}
        ' "$activities_file" > "$activity_block" || true
        timeout 8s adb shell dumpsys window windows > "$windows_file" 2>&1 || true

        if grep -Eq "firstWindowDrawn=true|reportedDrawn=true" "$activity_block"; then
          if ! grep -Eq "windows=\[Window\{[^]]*Splash Screen ${PACKAGE//./\\.}" "$activity_block"; then
            printf 'FIRST_WINDOW_DRAWN\n' > "reports/android-launch-${pass}-first-draw.txt"
            printf '%s' "$pid"
            return 0
          fi
        fi
  ...
  if [[ "$resumed_seen" -eq 1 ]]; then return 5; fi
  if [[ "$process_seen" -eq 1 ]]; then return 2; fi
  return 1
}
...
printf 'no\n' | avdmanager create avd --force --name "$AVD_NAME" --package "$SYSTEM_IMAGE" --device "pixel_5" > reports/android-avd-create.log 2>&1
adb kill-server || true
adb start-server
"$ANDROID_HOME/emulator/emulator" -avd "$AVD_NAME" -no-window -no-audio -no-boot-anim -no-snapshot-load -no-snapshot-save -gpu swiftshader_indirect -camera-back none -camera-front none -memory 3072 -cores 2 -no-metrics > reports/android-emulator.log 2>&1 &
EMU_PID=$!
...
for ATTEMPT in $(seq 1 120); do
  ...
  if [[ "$STATE" == "device" && "$BOOT" == "1" ]]; then BOOTED=1; break; fi
  if ! kill -0 "$EMU_PID" 2>/dev/null; then tail -n 200 reports/android-emulator.log || true; echo 'Emulator process exited before Android boot completed' >&2; exit 1; fi
  sleep 5
done

if [[ "$BOOTED" != "1" ]]; then capture_diagnostics 'EMULATOR_BOOT_TIMEOUT'; echo 'Android emulator failed to reach sys.boot_completed=1' >&2; exit 1; fi
if ! INITIAL_BOOT_ID="$(read_boot_id 8 2)"; then capture_diagnostics 'EMULATOR_BOOT_ID_UNAVAILABLE'; echo 'Android emulator booted but stable boot identity could not be read' >&2; exit 1; fi
...
test -s "$APK"
if ! timeout 90s adb install -r "$APK" | tee reports/android-install.log; then capture_diagnostics 'APK_INSTALL_TIMEOUT_OR_FAILURE'; exit 1; fi
...
for PASS in 1 2 3; do
  if ! wait_for_adb_device 10 2; then capture_diagnostics "LAUNCH_${PASS}_ADB_OFFLINE_BEFORE_START"; exit 1; fi
  timeout 10s adb shell am force-stop "$PACKAGE" || true
  sleep 1
  if ! start_activity_command "$PASS"; then capture_diagnostics "LAUNCH_${PASS}_START_COMMAND_FAILURE"; cat "reports/android-launch-${PASS}.log" || true; exit 1; fi
  ...
  if [[ "$READY_STATUS" -eq 4 ]]; then capture_diagnostics "LAUNCH_${PASS}_EMULATOR_EXITED"; exit 1
  elif [[ "$READY_STATUS" -eq 3 || "$READY_STATUS" -eq 1 ]]; then capture_diagnostics "LAUNCH_${PASS}_PROCESS_DIED"; exit 1
  elif [[ "$READY_STATUS" -eq 2 ]]; then capture_diagnostics "LAUNCH_${PASS}_NOT_RESUMED"; exit 1
  elif [[ "$READY_STATUS" -eq 5 ]]; then capture_diagnostics "LAUNCH_${PASS}_FIRST_WINDOW_NOT_DRAWN"; echo "BARSA MainActivity resumed but first app window did not draw during pass $PASS" >&2; exit 1
  fi

  printf 'LAUNCH_%s_PID=%s\n' "$PASS" "$PID" | tee "reports/android-launch-${PASS}-pid.txt"
  grep -qx 'FIRST_WINDOW_DRAWN' "reports/android-launch-${PASS}-first-draw.txt"

  if ! CURRENT_BOOT_ID="$(read_boot_id 8 2)"; then capture_diagnostics "LAUNCH_${PASS}_BOOT_ID_UNAVAILABLE"; exit 1; fi
  printf 'LAUNCH_%s_BOOT_ID=%s\n' "$PASS" "$CURRENT_BOOT_ID" | tee "reports/android-launch-${PASS}-boot-id.txt"
  if [[ "$CURRENT_BOOT_ID" != "$INITIAL_BOOT_ID" ]]; then capture_diagnostics "LAUNCH_${PASS}_DEVICE_REBOOTED"; exit 1; fi

  timeout 15s adb logcat -d -v time > "reports/android-logcat-${PASS}.txt" 2>&1 || true
  if has_app_crash_signature "reports/android-logcat-${PASS}.txt"; then capture_diagnostics "LAUNCH_${PASS}_CRASH_OR_ANR_SIGNATURE"; exit 1; fi
done
...
if ! wait_for_adb_device 12 2; then capture_diagnostics 'ADB_OFFLINE_BEFORE_FINAL_LOGCAT'; exit 1; fi
...
if has_app_crash_signature reports/android-logcat.txt; then capture_diagnostics 'CRASH_OR_ANR_SIGNATURE'; exit 1; fi
