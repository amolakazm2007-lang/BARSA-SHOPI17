...
test('cold-start gate requires an actually drawn app window, not merely a resumed Activity', async () => {
  const script = await source();
  assert.match(script, /firstWindowDrawn=true\|reportedDrawn=true/);
  assert.match(script, /FIRST_WINDOW_DRAWN/);
  assert.match(script, /android-launch-\$\{pass\}-first-draw\.txt/);
  assert.match(script, /LAUNCH_\$\{PASS\}_FIRST_WINDOW_NOT_DRAWN/);
  assert.match(script, /MainActivity resumed but first app window did not draw/);
});
