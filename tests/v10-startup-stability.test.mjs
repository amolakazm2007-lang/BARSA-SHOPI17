...
test('release CI boots a real Android AVD and proves the bounded app-launch script executed', async () => {
  ...
  assert.doesNotMatch(gate, /adb shell am start\s+-W\b/);
  assert.match(gate, /timeout 12s adb shell am start -n/);
  ...
  assert.match(gate, /LAUNCH_\$\{PASS\}_ADB_OFFLINE_BEFORE_START/);
  assert.match(gate, /LAUNCH_\$\{PASS\}_PROCESS_DIED/);
  assert.match(gate, /LAUNCH_\$\{PASS\}_NOT_RESUMED/);
  assert.match(gate, /LAUNCH_\$\{PASS\}_DEVICE_REBOOTED/);
