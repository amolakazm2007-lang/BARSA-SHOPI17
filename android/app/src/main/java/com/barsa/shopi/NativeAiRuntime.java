...
public final class NativeAiRuntime implements AutoCloseable {
    ...
    public synchronized void releaseSessions(){List<SessionHolder> closing=new ArrayList<>(sessions.values());sessions.clear();for(SessionHolder holder:closing){synchronized(holder){holder.close();}}}
