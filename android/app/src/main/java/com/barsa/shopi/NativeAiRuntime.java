...
public final class NativeAiRuntime implements AutoCloseable {
    ...
    private static final class SessionHolder implements AutoCloseable {
        ...
        float[] concatScratch(int length) {
            if (concatScratch == null || concatScratch.length < length) concatScratch = new float[length];
            return concatScratch;
        }
    ...
    public synchronized boolean registerModelFile(String modelId, File incoming, String sha256) {
        ...
        String requestedSha = sha256 == null ? "" : sha256.trim();
        String expectedSha = normalizeSha256(sha256);
        if (!requestedSha.isEmpty() && expectedSha.isEmpty()) return false;
        try {
            if (tmp.exists()) tmp.delete();
            copyFile(incoming, tmp);
            if (!expectedSha.isEmpty() && !expectedSha.equalsIgnoreCase(sha256Hex(tmp))) { tmp.delete(); return false; }
            closeSession(safe);
            try { Files.move(tmp.toPath(), target.toPath(), StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.ATOMIC_MOVE); }
            catch (Exception atomicMoveFailed) { Files.move(tmp.toPath(), target.toPath(), StandardCopyOption.REPLACE_EXISTING); }
            if (target.isFile() && target.length() > 1024) { writeFingerprint(safe, expectedSha, target.length()); return true; }
            return false;
    ...
    public InferenceResult infer(String modelId, float[] input, int channels, int width, int height, int scale, float fidelity) throws Exception {
        ...
        String safeModelId = safeId(modelId);
        for (;;) {
            SessionHolder holder = getOrCreateSession(modelId);
            synchronized (holder) {
                if (sessions.get(safeModelId) != holder) continue;
    ...
    public InferenceResult inferRife(String modelId, float[] frame0, float[] frame1, int width, int height, float timestep) throws Exception {
        ...
        for (;;) {
            ...
            try {
                ...
                if(concat){int total=a.length+b.length;float[] both=holder.concatScratch(total);System.arraycopy(a,0,both,0,a.length);System.arraycopy(b,0,both,a.length,b.length);OnnxTensor t=OnnxTensor.createTensor(environment,FloatBuffer.wrap(both,0,total),new long[]{1,6,runH,runW});tensors.add(t);feeds.put(imageInputs.get(0).getKey(),t);}
    ...
    private static String normalizeSha256(String value) {
        String sha = value == null ? "" : value.trim().toLowerCase(Locale.ROOT);
